'''
script purpose:             visualization of LEE boxplots splitted into 4 time periods
outputs:                    time_period_LEE_separated.png
...
df_26=pd.read_csv(os.path.join(rootOut, f'time_periods_LEE_RCP26_{city}_2.csv'), sep=',') 
df_45=pd.read_csv(os.path.join(rootOut, f'time_periods_LEE_RCP45_{city}_2.csv'), sep=',')
df_85=pd.read_csv(os.path.join(rootOut, f'time_periods_LEE_RCP85_{city}_2.csv'), sep=',')

# Check format
# df_26.head()
# print(df_26.head())
# print(df_26.iloc[:,2:]) # data starts from 3 column on
...
axs[0].set_title(f'Average length of extreme days per year in {station} \n for four different time periods', size=12, pad=10)
axs[0].set_ylim(bottom=0, top =5) # Zurich: 5 / Basel: 5 
axs[1].set_ylim(bottom=0, top =5) # Zurich: 5 / Basel: 5 
axs[2].set_ylim(bottom=0, top =5) # Zurich: 5 / Basel: 5 
axs[0].yaxis.set_ticks(np.arange(0, 5, 1)) # Zurich: 5 / Basel: 5 
axs[1].yaxis.set_ticks(np.arange(0, 5, 1)) # Zurich: 5 / Basel: 5 
axs[2].yaxis.set_ticks(np.arange(0, 5, 1)) # Zurich: 5 / Basel: 5 
...
fig1.savefig(os.path.join(rootOut, 'z_figures/time_period_LEE_separated.png'), format='png', dpi=1000)
