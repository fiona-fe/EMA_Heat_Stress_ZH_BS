'''
script purpose:             visualization of NEE boxplots splitted into 4 time periods
outputs:                    time_period_NEE_separated.png
...
df_26=pd.read_csv(os.path.join(rootOut, f'time_periods_NEE_RCP26_{city}_2.csv'), sep=',') 
df_45=pd.read_csv(os.path.join(rootOut, f'time_periods_NEE_RCP45_{city}_2.csv'), sep=',')
df_85=pd.read_csv(os.path.join(rootOut, f'time_periods_NEE_RCP85_{city}_2.csv'), sep=',')


'''Boxplots number of events, 1981-2020, 2021-2050, 2051-2080, 2081-2099, separated'''
...
axs[0].set_title(f'Average number of extreme events per year in {station} \n for four different time periods', size=12, pad=10)
axs[0].set_ylim(bottom=0, top =13) # Zurich: 11 / Basel: 13 (MODIFY)
axs[1].set_ylim(bottom=0, top =13) # Zurich: 11 / Basel: 13 (MODIFY)
axs[2].set_ylim(bottom=0, top =13) # Zurich: 11 / Basel: 13 (MODIFY)
axs[0].yaxis.set_ticks(np.arange(0, 13, 2)) # Zurich: 11 / Basel: 13 (MODIFY)
axs[1].yaxis.set_ticks(np.arange(0, 13, 2)) # Zurich: 11 / Basel: 13 (MODIFY)
axs[2].yaxis.set_ticks(np.arange(0, 13, 2)) # Zurich: 11 / Basel: 13 (MODIFY)
...
fig1.savefig(os.path.join(rootOut, 'z_figures/time_period_NEE_separated.png'), format='png', dpi=1000)
