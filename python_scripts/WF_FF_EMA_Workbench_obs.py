'''
script purpose:             EMA with WBGT calculations using historic climate data (1981-2020) for RCP2.6/4.5/8.5    
outputs:                    'wbgt.csv', 
                            '**name**_runs.tar.gz',
                            'Outputs_py'
                            'outSeason',
...
# HDNs Model 
# Initializiing the main dictionary for a case study
caseStudyStns = {}
inputFolder = ''
TmaxFolder = ''
TminFolder = ''
...
root = r'/Users/erdtiger/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Studium/EClim/Publication_Thesis_fweibel/ema_case_Basel_HDNs/case2_Basel_obs/setup3'
...
def HDNs_Model (xRCP=None, xClimateModel=None, Xfactor1 = None, xCostDay = None, xRevenueDay = None,
    ...
    '''Step 11: building a database for each precipitation and temperature file in Climate folder and saving them in a list'''
    ...
    for k in range(len(caseStudyStns)):
        ...
        for j in range(climateModel, climateModel + 1, 1):
            ...
            todayHMD = round(dfhmd[k][dfhmdCol[j]].iloc[1],2) if(dfhmd[k][dfhmdCol[2*j]].iloc[1] != -99) else 0
            todayWND = dfwnd[k][dfwndCol[j]].iloc[1] if(dfwnd[k][dfwndCol[2*j]].iloc[1] != -99) else 0
            todaySLR = round(dfslr[k][dfslrCol[j]].iloc[1],2) if(dfslr[k][dfslrCol[2*j]].iloc[1] != -99) else 0

            '''threshold Temperatures C
            EMA_workbench_controler for the threshold of hott days and hot nights'''
            ...
            for i in range(2, simulationLength + 1, 1):
                ...
                todayTMPAVE = round((todayTMPMAX+todayTMPMIN)/2,2) if((todayTMPMAX+todayTMPMIN)/2 != -99) else 0
                todayHMD = round(dfhmd[k][dfhmdCol[j]].iloc[i],2) if(dfhmd[k][dfhmdCol[2*j]].iloc[i] != -99) else 0
                todayWND = dfwnd[k][dfwndCol[j]].iloc[i] if(dfwnd[k][dfwndCol[2*j]].iloc[i] != -99) else 0
                todaySLR = round(dfslr[k][dfslrCol[j]].iloc[i],2) if(dfslr[k][dfslrCol[2*j]].iloc[i] != -99) else 0
                ...
                '''3.) Carter et al., 2020 based on Bernard, 1999'''
                '''WBGT = 0.7*Tnwb + 0.2*Tg + 0.1*Ta'''
        ...
        '''Saving wbgt values in a csv file'''
        wbgt_df = pd.DataFrame(wbgt)
        wbgt_df.to_csv(r'/Users/erdtiger/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Studium/EClim/Publication_Thesis_fweibel/ema_case_Basel_HDNs/case2_Basel_obs/setup3/wbgt_maxT_Bernard.csv', index=False)
...
# Here we calculate seasonal tipping points
        for i in range(len(totalFiles)):
            ...
            # AveragesumRows: the averange nmber of days in a season with Hot snow condition(season 1981-1982 to 2098-2099)                                                                                                                         
            ...
            AveragesumRows = np.average(sumRows)
...
if __name__ == '__main__':
    ...
    model.levers = [
                    RealParameter("x7Wbgtthreshold", 26, 30) # default: ("x7Wbgtthreshold", 26, 30), defines WBGT threshold
                    ]
    ...
    # override some of the defaults of the model. Kwakkel (2017): 'Constants give the possibility to overwrite default values of parameters that are not uncertain'
    model.constants = [Constant("xRCP", 1), 
                       Constant("xClimateModel", 0)]    
...
save_results(results, r'/Users/erdtiger/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Studium/EClim/Publication_Thesis_fweibel/ema_case_Basel_HDNs/case2_Basel_obs/setup3_bernard_136_30/26_30_bernard/20250301_runs.tar.gz')
